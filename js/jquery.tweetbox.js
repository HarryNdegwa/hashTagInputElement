(function ($) {
	...
	!(function (a, b) {
		...
		if (
			...
			(d.android || d.ipad || d.iphone || d["windows phone"]) &&
				(d.mobile = !0),
	...
	$.fn.tweetbox = function (option) {
		if (typeof option == "object" || typeof option == "undefined") {
			...
			typeof methods[option] == "function"
		) {
	...
	var methods = {
		init: function (settings) {
			...
			this.settings = $.extend(
				{
					...
					highlight: [
						...
						{
							match: /(http|https):\/\/(\w+:{0,1}\w*@)?(\S+)(:[0-9]+)?(\/|\/([\w#!:.?+=&%@!\-\/]))?/gi,
							class: "url",
						},
					],
					action: "tweet",
			...
			var sourceText =
				...
				stripTags($.trim($(this).html().replace(/<br>/g, "\n")));
			...
			$(this)
				.css({ overflow: "hidden", padding: "0px" })
				.html(this.$editor);
			...
			this.$button = $('<button class="tweet-box-button">')
				.css({
					position: "absolute",
					top: "5px",
					bottom: "5px",
				...
				.html(this.settings.button[this.settings.action])
				.click(function () {
					var text =
						rawText(self.$editor.html(), true) +
						self.settings.postfix;
					var avaiable = self.settings.limit - text.length;
					if (avaiable < 0)
						$(self).tweetbox(
							"info",
							'<span style="color:red">Please shorten your tweet</span>'
						);
					else if (typeof self.settings.cb == "function")
						self.settings.cb(text);
				});
			this.$info = $('<div class="tweet-box-info">')
				.css({
					position: "absolute",
					top: "5px",
					left: "1%",
				...
				.html("X Characters Left");
			this.$bottom = $('<div class="tweet-box-bottom">')
				.css(cover)
				.css({
					top: "auto",
					height: "40px",
		...
		highlight: function (keyCode) {
			...
			if (keyCode && keyCode == 13 && start == 0) {
				...
				keyCode &&
				keyCode == 13 &&
				start == this.previousText.length + 1
			) {
			...
			if (keyCode && keyCode == 13 && $.browser.msie) {
				...
				$(this).tweetbox(
					"info",
					'<span style="color:white">Sorry, multiple lines not supported by IE.</span>'
				);
			} else
				$(this).tweetbox(
					"info",
					avaiable < 0
						? '<span style="color:red">' + avaiable + "</span>"
						: avaiable
				);
			formatted = text;
			for (var i = 0; i < this.settings.highlight.length; i++)
				formatted = formatted.replace(
					this.settings.highlight[i].match,
					function (m) {
						return (
							'<span class="' +
							self.settings.highlight[i].class +
							'">' +
							m +
							"</span>"
						);
